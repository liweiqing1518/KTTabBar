...
- (KTTabItem *)yp_tabItem {
    KTTabBar *tabBar = self.yp_tabBarController.tabBar;
    if (!tabBar) {
        return nil;
    }
...
- (KTTabBarController *)yp_tabBarController {
    return (KTTabBarController *)self.parentViewController;
}
...
@interface KTTabBarController ()<UIScrollViewDelegate, YPTabContentScrollViewDelegate> {
...
- (void)setViewControllers:(NSArray *)viewControllers {
    ...
    for (UIViewController *controller in _viewControllers) {
        ...
        KTTabItem *item = [KTTabItem buttonWithType:UIButtonTypeCustom];
...
- (void)yp_tabBar:(KTTabBar *)tabBar didSelectedItemAtIndex:(NSUInteger)index {
...
- (void)imgAnimate:(KTTabBar *)tabBar{
    
    UIView *view=tabBar.selectedItem;//.subviews[0];
