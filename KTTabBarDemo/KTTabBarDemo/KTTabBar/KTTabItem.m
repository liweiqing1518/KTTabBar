...
+ (instancetype)buttonWithType:(UIButtonType)buttonType {
    KTTabItem *item = [super buttonWithType:buttonType];
    [item setup];
    return item;
}
...
- (void)setup {
    ...
    _badgeStyle = KTTabItemBadgeStyleNumber;
    self.badge = 0;
}
...
- (void)setBadgeStyle:(KTTabItemBadgeStyle)badgeStyle {
    _badgeStyle = badgeStyle;
    [self updateBadge];
}

- (void)updateBadge {
    if (self.badgeStyle == KTTabItemBadgeStyleNumber) {
        if (self.badge == 0) {
    ...
    } else if (self.badgeStyle == KTTabItemBadgeStyleDot) {
