 ...
 *  是否能切换到指定index
 */
- (BOOL)yp_tabBar:(KTTabBar *)tabBar shouldSelectItemAtIndex:(NSUInteger)index;

/**
 *  将要切换到指定index
 */
- (void)yp_tabBar:(KTTabBar *)tabBar willSelectItemAtIndex:(NSUInteger)index;
...
@interface KTTabBar : UIButton
/**
 *  TabItems，提供给YPTabBarController使用，一般不手动设置此属性
 */
@property (nonatomic, copy) NSArray <KTTabItem *> *items;
 ...
 */
- (void)setSpecialItem:(KTTabItem *)item
    afterItemWithIndex:(NSUInteger)index
            tapHandler:(void (^)(KTTabItem *item))handler;
