...
@property (nonatomic, strong) KTTabItem *specialItem;
@property (nonatomic, copy) void (^specialItemHandler)(KTTabItem *item);
...
- (void)setItems:(NSArray *)items {
    ...
    // 初始化每一个item
    for (KTTabItem *item in self.items) {
...
- (void)updateItemsFrame {
    ...
    if (self.scrollView) {
        ...
        for (NSUInteger index = 0; index < self.items.count; index++) {
            KTTabItem *item = self.items[index];
            CGFloat width = 0;
            // item的宽度为一个固定值
            if (self.itemWidth > 0) {
...
- (void)setSelectedItemIndex:(NSUInteger)selectedItemIndex {
    ...
    if (_selectedItemIndex != NSNotFound) {
        KTTabItem *oldSelectedItem = self.items[_selectedItemIndex];
    ...
    KTTabItem *newSelectedItem = self.items[selectedItemIndex];
...
- (void)updateSelectedBgFrameWithIndex:(NSUInteger)index {
    ...
    CGFloat height = 1;//item.frameWithOutTransform.size.height - self.itemSelectedBgInsets.top - self.itemSelectedBgInsets.bottom;
...
- (void)tabItemClicked:(KTTabItem *)item {
    self.selectedItemIndex = item.index;
}

- (void)specialItemClicked:(KTTabItem *)item {
    if (self.specialItemHandler) {
        self.specialItemHandler(item);
    }
}

- (KTTabItem *)selectedItem {
...
- (void)setSpecialItem:(KTTabItem *)item afterItemWithIndex:(NSUInteger)index tapHandler:(void (^)(KTTabItem *item))handler {
...
- (void)updateSeperators {
    if (self.itemSeparatorColor) {
        ...
        [self.items enumerateObjectsUsingBlock:^(KTTabItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
            if (idx > 0) {
...
- (void)updateSubViewsWhenParentScrollViewScroll:(UIScrollView *)scrollView {
    ...
    KTTabItem *leftItem = self.items[leftIndex];
    KTTabItem *rightItem = nil;
